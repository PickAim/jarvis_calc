...
def frequency_calc(costs: ndarray[float], n_samples: int) -> tuple[list[float], list[int]]:
...
def get_frequency_stats(cost_data: list[float], n_samples: int) -> tuple[list[float], list[int]]:
    ...
    if len(cost_data) <= 0:
        return [], []
    cost_data.sort()
    base_keys, base_frequencies = frequency_calc(np.array(cost_data), n_samples)
    keys = base_keys.copy()
        ...
        if 0 < interesting_part_ind < len(frequencies):
            ...
            for cost in cost_data:
                if cost < keys[interesting_part_ind - 1]:
                    ...
                    break
            keys, frequencies = frequency_calc(np.array(left_costs), n_samples - 1)
